...
export const remoteApi = {
  remoteStatus: () => invokeLocal<RemoteStatus>("remote_status"),
  remoteSetEnabled: (enabled: boolean) =>
    invokeLocal<RemoteStatus>("remote_set_enabled", { enabled }),
  /** Change the listen port; a live listener restarts on it. Rejects a port
   *  that can't be bound, leaving the current one running. */
  remoteSetPort: (port: number) => invokeLocal<RemoteStatus>("remote_set_port", { port }),
