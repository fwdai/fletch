// Absolute-path arithmetic for the folder browsers — the desktop's remote
// picker, the phone's Add Project sheet — and the mock host's fake filesystem.
// The host answers `list_dir` with the absolute, tilde-expanded `base` it
// actually read, so every path built here is anchored on that rather than on
