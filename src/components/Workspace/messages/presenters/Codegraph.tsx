...
/** Pull the operation out of a fully-qualified codegraph MCP tool name,
 *  e.g. "mcp__codegraph__codegraph_explore" -> "explore". Falls back to the
 *  trailing segment for any shape we don't recognize. */
function codegraphOp(name: string): string {
  const tail = name.split("__").pop() ?? name;
  return tail.replace(/^codegraph_/, "");
}
