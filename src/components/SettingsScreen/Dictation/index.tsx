...
/** Settings › General › Dictation: opt into the local Whisper engine instead of
 *  Apple's recognizer, and pick which model it runs. The opt-in and the weights
 *  are separate states — the toggle flips immediately and the model downloads
 *  behind it — so the chooser below is the only place that says whether
 *  dictation can actually run locally yet. A group inside General rather than
 *  its own section: dictation is how you talk to the composer, so it reads
 *  next to the composer's other controls. macOS-only, like the recognizer it
 *  replaces; General renders it only there. */
