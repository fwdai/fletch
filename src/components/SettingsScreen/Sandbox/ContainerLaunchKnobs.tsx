...
const LAUNCH_FIELDS = [
  {
    ...
    sub: (_: Runtime) =>
      "Custom agent image. Needs claude and git on PATH. Blank uses Fletch's image.",
    placeholder: "fletch-agent (built-in)",
  },
  {
    key: "memory",
    title: "Memory limit",
    sub: (r: Runtime) => `Passed to ${RUNTIMES[r].bin} run --memory. Blank uses the default (4g).`,
    placeholder: "4g",
  },
  {
    key: "cpus",
...
/** Container-sandbox launch knobs for one runtime. These persist to the
 ...
 *  harmless otherwise. */
export function ContainerLaunchKnobs({ runtime, last }: { runtime: Runtime; last?: boolean }) {
