    ...
    fn the_dial_and_the_button_land_in_the_same_place() {
        assert_eq!(autonomy::accept_landing(false, false, false), Landing::Board);
        ...
        assert_eq!(autonomy::accept_landing(true, false, true), Landing::HeldBack);
        assert_eq!(autonomy::accept_landing(false, true, true), Landing::HeldBack);
    ...
    fn a_hold_pauses_an_item_without_sealing_it_to_proposals() {
        ...
        let rulings::Ruling::Updated { item, .. } = rulings::accept_proposal(&conn, &p.id).unwrap() else {
    ...
    fn accepting_a_discard_keeps_the_row_as_rejected() {
        ...
        let rulings::Ruling::Updated { item, event } = rulings::accept_proposal(&conn, &p.id).unwrap() else {
            panic!("expected Updated");
        };
