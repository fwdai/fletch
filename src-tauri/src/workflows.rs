...
//! per-step checkouts, the step-boundary commit, gate probes, and the final
//! push + PR. Checkout paths are resolved server-side from `(agent_id, subdir)`
//! via `crate::workspace::repo_checkout_path`, so callers pass the ids from the
...
#[tauri::command]
pub async fn workflow_save_run_step(
    step: Value,
    db: tauri::State<'_, Db>,
) -> Result<Value, String> {
    let st = &step;
