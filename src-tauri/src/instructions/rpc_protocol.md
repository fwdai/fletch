...
- `open_pr` — creates your branch (if you don't have one yet), pushes it, and opens a pull request against your base branch. `args.branch` is the branch name to create — choose a conventional, descriptive name like `fix/login-crash`, `feat/csv-export`, or `chore/bump-deps` (no `quorum/` prefix). Required the first time you push; omit it later to keep using your current branch (pushing again updates the same PR). Pass a *new* `args.branch` to cut a fresh branch for a second PR from the same worktree. `args.title` and `args.body` set the PR title and description; omit `title` to auto-fill from your commits.
- `git_push` — pushes your branch to origin (sets the upstream on first push). When you're already on a branch, no args are needed — use it to update an existing pull request after committing fixes. When you don't have a branch yet, pass `args.branch` (a conventional name like `fix/…`) so it can be created at your current commit before pushing.
