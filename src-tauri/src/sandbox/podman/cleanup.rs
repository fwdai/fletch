...
//! podman has one container store per machine *endpoint* — a machine's rootless
//! and rootful connections are two of them — and launches pin themselves to the
//! connection they resolved at the time (see [`super::machine`]). Asking only
//! the current default would strand containers on every other endpoint, so both
//! sweeps run once per machine connection.
//!
