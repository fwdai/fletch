...
//!
//! Shared by the Docker and Podman engines — both emit the same mount/env
//! argv after their own binary; only the CLI that consumes it differs.
...
/// here, and the `CredentialsFile` auth chain needs that write to land on the
/// host. Mounted read-write on top of the read-only dir. The name is shared
/// with seatbelt via [`CLAUDE_CREDENTIALS_FILE`] so the engines can't drift.
pub const CREDENTIALS_FILE: &str = CLAUDE_CREDENTIALS_FILE;
...
/// grants the real dirs as writable islands) so the engines can't drift.
...
/// Shared with seatbelt via [`CLAUDE_PROJECTS_SUBDIR`] so the engines can't
/// drift.
...
/// one variant per supported provider. Each variant holds exactly its own
/// provider's data and [`run_args`] matches once on the whole thing. Claude's
/// read-only-except-carve-outs treatment is unique; the others are read-write
/// binds at identical host paths, differing only in which dirs they mount and
/// which env var (if any) they forward.
pub enum ProviderMounts<'a> {
...
pub struct RunSpec<'a> {
    ...
    pub image: &'a str,
    pub agent_bin: &'a str,
    /// Auth var *names* the chain resolved, each forwarded with a bare
    /// `-e NAME` so its value (set on the container CLI process env) never
    /// appears in argv. Only the resolved set is forwarded: an ambient
    /// credential the chain didn't pick must not reach the container and
    /// override the resolved login.
    pub auth_vars: &'a [&'a str],
}

/// The `run` argv (everything after the container CLI binary), ending with
...
pub fn run_args(spec: &RunSpec<'_>) -> Vec<String> {
    ...
    // Bare `-e NAME` forwards from the container CLI's own environment without
...
/// `-v`. The dir itself must exist or the runtime would materialize it
/// root-owned; each overlay target must exist *inside* it too, because the
/// overlay ([`push_claude_config_mount`]) mounts onto that subpath and the RO
/// parent bind can't grow a fresh mountpoint at run time. The overlays are the
...
pub fn prepare_config_mount_dir(dir: &Path) -> Result<()> {
...
///   is absent (a bare `-v` on a missing source would have the runtime create a
...
///
/// Every overlay is pushed *after* the RO dir mount so the runtime layers it
/// on top.
